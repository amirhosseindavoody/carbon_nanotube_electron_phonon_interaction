module cnt_scattering_exciton_phonon_mod
	implicit none
	private
	public :: cnt_exction_phonon_scattering_rate_emission, cnt_exction_phonon_scattering_rate_emission_objective
...
contains
						...
						do ix_2=1,currcnt%ikr_high-currcnt%ikr_low+1
							...
							do k = 1, n_final_state
								...
								do ikr = (currcnt%ikr_low+abs(iq_ph_r)),(currcnt%ikr_high-abs(iq_ph_r))
									...
									matrix_element_exciton = matrix_element_exciton + currcnt%Psi0_A2(ikr,ix,iKcm)*conjg(currcnt%Psi0_A2(ikr-iq_ph_r,ix_2,iKcm_2))*matrix_element_electron/dcmplx(2.d0)
									...
									matrix_element_exciton = matrix_element_exciton + currcnt%Psi0_A2(ikr,ix,iKcm)*conjg(currcnt%Psi0_A2(ikr+iq_ph_r,ix_2,iKcm_2))*matrix_element_hole/dcmplx(2.d0)
	...
	! calculate the exciton-phonon scattering rates due to phonon emission
	!***************************************************************************
	subroutine cnt_exction_phonon_scattering_rate_emission_objective(currcnt, i_exciton, f_exciton)
							...
							do k = 1, n_final_state
								...
								do ikr = (currcnt%ikr_low+abs(iq_ph_r)),(currcnt%ikr_high-abs(iq_ph_r))
									...
									do imu_r = 1,i_exciton%n_mu_r
										...
										call graphene_electron_phonon_matrix_element(matrix_element_electron, kc, q_ph, ib, currcnt%a1, currcnt%a2)
										matrix_element_exciton = matrix_element_exciton + i_exciton%psi(ikr,ix,iKcm,imu_r)*conjg(f_exciton%psi(ikr-iq_ph_r,ix_2,iKcm_2,imu_r))*matrix_element_electron

										! valence band electron scattering terms
										kv = dble(mu_r-mu_cm) * currcnt%K1 + (dble(ikr*currcnt%dk_dkx_ratio-iKcm)) * currcnt%dkx * currcnt%K2
										call graphene_electron_phonon_matrix_element(matrix_element_hole, kv, q_ph, ib, currcnt%a1, currcnt%a2)
										matrix_element_exciton = matrix_element_exciton + i_exciton%psi(ikr,ix,iKcm,imu_r)*conjg(f_exciton%psi(ikr+iq_ph_r,ix_2,iKcm_2,imu_r))*matrix_element_hole

									enddo

									! !! this is the sum over basis functions |+mu_r,+kr,K>
									! ! conduction band electron scattering terms
									! kc = dble(i_exciton%mu_r(1)+mu_cm) * currcnt%K1 + (dble(ikr*currcnt%dk_dkx_ratio+iKcm)) * currcnt%dkx * currcnt%K2
									! call graphene_electron_phonon_matrix_element(matrix_element_electron, kc, q_ph, ib, currcnt%a1, currcnt%a2)
									! matrix_element_exciton = matrix_element_exciton + i_exciton%psi(ikr,ix,iKcm,1)*conjg(f_exciton%psi(ikr-iq_ph_r,ix_2,iKcm_2,1))*matrix_element_electron
									! ! valence band electron scattering terms
									! kv = dble(i_exciton%mu_r(1)-mu_cm) * currcnt%K1 + (dble(ikr*currcnt%dk_dkx_ratio-iKcm)) * currcnt%dkx * currcnt%K2
									! call graphene_electron_phonon_matrix_element(matrix_element_hole, kv+q_ph, q_ph, ib, currcnt%a1, currcnt%a2)
									! matrix_element_exciton = matrix_element_exciton + i_exciton%psi(ikr,ix,iKcm,1)*conjg(f_exciton%psi(ikr+iq_ph_r,ix_2,iKcm_2,1))*matrix_element_hole
									!
									! !! this is the sum over basis functions |-mu_r,-kr,K>
									! ! conduction band electron scattering terms
									! kc = dble(i_exciton%mu_r(2)+mu_cm) * currcnt%K1 + (-dble(ikr*currcnt%dk_dkx_ratio+iKcm)) * currcnt%dkx * currcnt%K2
									! call graphene_electron_phonon_matrix_element(matrix_element_electron, kc, q_ph, ib, currcnt%a1, currcnt%a2)
									! matrix_element_exciton = matrix_element_exciton + i_exciton%psi(ikr,ix,iKcm,1)*conjg(f_exciton%psi(ikr-iq_ph_r,ix_2,iKcm_2,1))*matrix_element_electron
									! ! valence band electron scattering terms
									! kv = dble(i_exciton%mu_r(2)-mu_cm) * currcnt%K1 + (-dble(ikr*currcnt%dk_dkx_ratio-iKcm)) * currcnt%dkx * currcnt%K2
									! call graphene_electron_phonon_matrix_element(matrix_element_hole, kv+q_ph, q_ph, ib, currcnt%a1, currcnt%a2)
									! matrix_element_exciton = matrix_element_exciton + i_exciton%psi(ikr,ix,iKcm,1)*conjg(f_exciton%psi(ikr+iq_ph_r,ix_2,iKcm_2,1))*matrix_element_hole
		...
		write(log_input,'(A)') "Exciton-phonon scattering rates due to phonon emission calculated and saved!"
		call write_log(log_input)
	end subroutine cnt_exction_phonon_scattering_rate_emission_objective

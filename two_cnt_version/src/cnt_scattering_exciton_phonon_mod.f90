module cnt_scattering_exciton_phonon_mod
	implicit none
	private
	public :: cnt_exciton_phonon_scattering_rate_emission
...
contains
	...
	subroutine cnt_electron_phonon_scattering_rate_emission(currcnt)
		...
		integer :: mu_ph, iq_ph
		integer :: ib
		integer :: n_initial_state
		integer, dimension(:), allocatable :: initial_state_idx
		...
		do i=1,energy_mesh_size
			do ix=1,currcnt%ikr_high-currcnt%ikr_low+1
				...
				do j=1,n_initial_state
					...
					K_cm = dble(mu_cm) * currcnt%K1 + dble(iK_cm) * currcnt%dkx * currcnt%K2
					do ib=1,6
						do ix_2=1,currcnt%ikr_high-currcnt%ikr_low+1
							mu_ph = 2*(mu_cm-mu_cm_2)
							tmp_real_array_1 = currcnt%Ex0_A2(ix_2,:)-currcnt%Ex0_A2(ix,iKcm)+currcnt%omega_phonon(mu_ph,ik_e+currcnt%ikc_max:ik_e+currcnt%ikc_min:-1,ib)
	...
	!***************************************************************************
	! - this subroutine calculates the phonon energy dispersion that is used
	!   in this calculating the exciton scattering rates.
	!***************************************************************************
	subroutine cnt_phonon_dispersion(currcnt, dq, iq_max, iq_min, mu_min, mu_max, save)
		use cnt_class, only : cnt
		...
		type(cnt), intent(inout) :: currcnt
		real*8, intent(in) :: dq
		integer, intent(in) :: iq_max, iq_min, mu_min, mu_max
		...
		character(len=1000) :: filename
		logical :: save_dispersion

		!***********************************************************************
		! calculate the phonon dispersion
		allocate(k_vec(iq_min:iq_max))
		if(allocated(currcnt%omega_phonon)) deallocate(currcnt%omega_phonon)
		allocate(currcnt%omega_phonon(mu_min:mu_max,iq_min:iq_max,6))
		...
		do mu=mu_min,mu_max
			do iq=iq_min,iq_max
				q=dble(mu)*currcnt%K1+dble(iq)*dq*currcnt%K2
		...
		if(present(save)) save_dispersion = save

		if(save_dispersion) then
			write(filename,'(A)') trim(currcnt%name)//".phonon_k_vector.dat"
			open(unit=100,file=trim(filename),status="unknown")

			do iq=lbound(k_vec,dim=1),ubound(k_vec,dim=1)

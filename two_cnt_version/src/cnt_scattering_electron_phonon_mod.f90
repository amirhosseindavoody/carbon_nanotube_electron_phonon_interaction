module cnt_scattering_electron_phonon_mod
	implicit none
	private
	public :: cnt_electron_phonon_scattering_rate_emission, cnt_electron_phonon_scattering_rate_absorption

	real*8, private :: energy_mesh_min, energy_mesh_max
	real*8, private :: energy_mesh_length = 2.0d0
	integer, private :: energy_mesh_size = 200

contains

	!**************************************************************************************************************************
	! calculate the electron-phonon scattering rates
	!**************************************************************************************************************************
	...
	subroutine cnt_electron_phonon_scattering_rate_emission(currcnt)
		...
		allocate(electron_phonon_scattering_rate(energy_mesh_size))
					...
					do ib=1,6
						do mu_e_2=1-currcnt%Nu/2,currcnt%Nu/2
							...
							do k = 1, n_scattering_state
								...
								electron_phonon_scattering_rate(i) = electron_phonon_scattering_rate(i) + (1.d0+1.d0/(exp(omega_tmp/temperature/kb)-1.d0)) * ((abs(f_tilde_1(ib) + f_tilde_2(ib)))**2)/(abs(-Ec_derivative+omega_phonon_derivative)*omega_tmp/hb)
		...
		do i = 1, energy_mesh_size
			write(100,'(SP, E20.8)', advance='no') electron_phonon_scattering_rate(i)
		enddo
					...
					do ib=1,6
						do mu_e_2=1-currcnt%Nu/2,currcnt%Nu/2
							...
							do k = 1, n_scattering_state
								...
								electron_phonon_scattering_rate(i) = electron_phonon_scattering_rate(i) + (1.d0/(exp(omega_tmp/temperature/kb)-1.d0)) * ((abs(f_tilde_1(ib) + f_tilde_2(ib)))**2)/(abs(-Ec_derivative+omega_phonon_derivative)*omega_tmp/hb)

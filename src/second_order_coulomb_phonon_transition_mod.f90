...
module second_order_coulomb_phonon_transition_mod
	implicit none
	private
	public :: calculate_second_order_transition_rates_exciton_integration, calculate_second_order_transition_rates_phonon_integration
...
contains
	...
	subroutine calculate_second_order_transition_rates_phonon_integration (cnt_1,cnt_2)
		use constants_mod
		use cnt_class, only: cnt, exciton
		use cnt_phonon_mod, only: cnt_phonon_dispersion
		use cnt_scattering_exciton_phonon_mod, only: cnt_exciton_phonon_matrix_element
	...
	end subroutine calculate_second_order_transition_rates_phonon_integration
	...
	subroutine calculate_second_order_transition_rates_maybe_optimized (cnt_1,cnt_2)
		use constants_mod
		use cnt_class, only: cnt, exciton
		use cnt_phonon_mod, only: cnt_phonon_dispersion
		use cnt_scattering_exciton_phonon_mod, only: cnt_exciton_phonon_matrix_element
...
! 												if (abs(f_exciton%ex(ix2,iKcm2)-m_exciton%ex(ix_m,iKcm_m)) .gt. 1.d-2*min(abs(f_exciton%ex(ix2,iKcm2+1)-f_exciton%ex(ix2,iKcm2-1)),abs(m_exciton%ex(ix_m,iKcm_m+1)-m_exciton%ex(ix_m,iKcm_m-1)))) then
! 												if (abs(f_exciton%ex(ix2,iKcm2)-m_exciton%ex(ix_m,iKcm_m)) .le. 1.d-2*min(abs(f_exciton%ex(ix2,iKcm2+1)-f_exciton%ex(ix2,iKcm2-1)),abs(m_exciton%ex(ix_m,iKcm_m+1)-m_exciton%ex(ix_m,iKcm_m-1)))) then
...
! 													min_delta_energy = min(min_delta_energy, abs(dcmplx(i_exciton%ex(ix1,iKcm1)-m_exciton%ex(ix_m,iKcm_m)-omega_tmp)+ i1*dcmplx(0.4d0*eV)))
! 													write(*,'(A , I0, A, I0, A, I0, A, I0)') "j=" , j, "/", n_initial_state, " k=", k, "/", n_mid_state
! 													write(*, '(A, I0, A, I0, A, I0, A, I0, A, I0, A, I0, A, I0, A, I0)') "ix1=", ix1, " ix2=", ix2, " ix_m=", ix_m, " ib=", ib, " iKcm1=", iKcm1, " iKcm2=", iKcm2, " iKcm_m=", iKcm_m, " iq_ph=", iq_ph
! 													write(*,'(A, E8.2)') "matrix_element=", abs(matrix_element)
! 													write(*, '(A)') "------------"
...
! 			scattering_rate(i) = scattering_rate(i) * f_exciton%dKcm / (2*pi*hb)
	...
	end subroutine calculate_second_order_transition_rates_maybe_optimized

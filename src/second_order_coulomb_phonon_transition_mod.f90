...
module second_order_coulomb_phonon_transition_mod
	implicit none
	private
	public :: calculate_phonon_emission_Coulomb_coupling_transition_rates, calculate_Coulomb_coupling_phonon_emission_transition_rates, calculate_phonon_emission_Coulomb_coupling_parallel
...
contains
	...
	subroutine calculate_phonon_emission_Coulomb_coupling_transition_rates (cnt_1,cnt_2)
		...
		do i = 1, energy_mesh_size
			...
			write(log_input, '(A, A, A, A, I0, A, I0, A, E8.2)') trim(i_exciton%name), " to ", trim(f_exciton%name), " phonon emission + Coulomb coupling second-order exciton scattering rate(", i, "/", energy_mesh_size, ")=", scattering_rate(i)
			call write_log(log_input)
		...
		write(filename,'(A, A, A, A, A)') "phonon_emission_Coulomb_coupling_scattering_rate.", trim(i_exciton%name), "_to_", trim(f_exciton%name), ".dat"
	...
	subroutine calculate_Coulomb_coupling_phonon_emission_transition_rates (cnt_1,cnt_2)
		use constants_mod
		use cnt_class, only: cnt, exciton
		use cnt_phonon_mod, only: cnt_phonon_dispersion
		use cnt_scattering_exciton_phonon_mod, only: cnt_exciton_phonon_matrix_element
		...
		do i = 1, energy_mesh_size
								...
								do k=1, n_final_state
									...
									if ((iKcm_m .ge. m_exciton%iKcm_min) .and. (iKcm_m .le. m_exciton%iKcm_max)) then
										...
										do ix_m = 1,1 !m_exciton%nx
											...
											coulomb_matrix_element = Q_tilde * geometric_matrix_element * (A_u**2) /(4.d0*(pi**2)*cnt_1%radius*cnt_2%radius)
											matrix_element = phonon_matrix_element * coulomb_matrix_element / (dcmplx(i_exciton%ex(ix1,iKcm1)-m_exciton%ex(ix_m,iKcm_m))+ i1*dcmplx(0.06d0*eV))
											total_matrix_element = total_matrix_element + matrix_element
			...
			write(log_input, '(A, A, A, A, I0, A, I0, A, E8.2)') trim(i_exciton%name), " to ", trim(f_exciton%name), " Coulomb coupling + phonon emission second-order exciton scattering rate(", i, "/", energy_mesh_size, ")=", scattering_rate(i)
			call write_log(log_input)
		...
		write(filename,'(A, A, A, A, A)') "Coulomb_coupling_phonon_emission_scattering_rate.", trim(i_exciton%name), "_to_", trim(f_exciton%name), ".dat"
		open(unit=100,file=trim(filename),status="unknown")
	...
	end subroutine calculate_Coulomb_coupling_phonon_emission_transition_rates
	...
	subroutine calculate_phonon_emission_Coulomb_coupling_parallel (cnt_1,cnt_2)
		...
		do i = 1, energy_mesh_size
			...
			write(log_input, '(A, A, A, A, I0, A, I0, A, E8.2)') trim(i_exciton%name), " to ", trim(f_exciton%name), " parallel geometry, phonon emission + Coulomb coupling, second-order exciton scattering rate(", i, "/", energy_mesh_size, ")=", scattering_rate(i)
			call write_log(log_input)

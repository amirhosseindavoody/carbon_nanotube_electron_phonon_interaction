...
contains
	...
	subroutine calculate_transition_table (cnt1,cnt2)
		...
		call write_log(new_line('A')//"************** Start calculating transitionTable ****************")
		...
		write(log_input, '("c2cMin[nm] = ", F4.1)') c2cMin*1.d9
		call write_log(trim(log_input))
		write(log_input, '("c2cMax[nm] = ", F4.1)') c2cMax*1.d9
		call write_log(trim(log_input))
		write(log_input, '("nC2C = ", I3.3)') nC2C
		...
		call write_log(trim(log_input))
		write(log_input, '("thetaMin = ", I3.3)') nint(thetaMin*180/pi)
		call write_log(trim(log_input))
		write(log_input, '("thetaMax = ", I3.3)') nint(thetaMax*180/pi)
		call write_log(trim(log_input))
		write(log_input, '("nTheta = ", I3.3)') nTheta
		call write_log(trim(log_input))
		...
		do ic2c = 1, nc2c
			...
			do iTheta = 1, nTheta
				...
				if ((cnt1%length .lt. huge(1.d0)) .and. (cnt2%length .lt. huge(1.d0))) then
					write(log_input,*) 'Calculating finite transition rate: iTheta=', iTheta, ', nTheta=', nTheta, 'iC2C=', ic2c, ', nC2C=', nc2c
				...
				! calculate exciton transfer rate for infinitely long CNTs
					...
					write(log_input,*) 'Calculating infinite transition rate: iTheta=', iTheta, ', nTheta=', nTheta, 'iC2C=', ic2c, ', nC2C=', nc2c
					call write_log(log_input)
